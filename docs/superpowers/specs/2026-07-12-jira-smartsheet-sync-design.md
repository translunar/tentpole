...
- **A third, narrow adapter: Jira-write.** Used exclusively by the human-invoked `fix apply` command (§5), never by the scheduled sync. The one-way contract is thus: *the sync never writes to Jira; the only Jira writes are human-reviewed, allowlisted `fix apply` executions attributed to the person who ran them.*
...
**Hygiene fixes:** a rule may name a `fix:` strategy from a core-shipped menu, in two classes: **mechanical** (parameters fully computable — e.g. `inherit_epic_fixversion`, ghost-row key matching) and **suggested** (ranked candidates a human must choose from — e.g. `suggest_epic_from_siblings`). Strategies are pure functions over the bundle emitting structured proposals `{issue, action, value, rationale, confidence}`, rendered wherever flags render. A separate human-invoked **`fix apply`** command (never part of the scheduled sync) walks proposals with per-change review and executes accepted ones through a Jira-write adapter, restricted to an allowlist of safe field edits (set fixVersion, set parent/epic, add link) — never transitions, never deletes — running as, and attributed to, the invoking human. Batch accept is allowed for mechanical fixes after review ("47 tasks inherit their epic's fixVersion — y/n/all").
...
- **Two-way sync of any kind.** Jira is the sole authoring surface for work data, permanently. (The `fix apply` command in §5 is not sync: it is a human making reviewed Jira edits with tool assistance.)
